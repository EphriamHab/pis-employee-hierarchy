...
const PositionList: React.FC<PositionListProps> = () => {
  ...
  const [positionToDelete, setPositionToDelete] = useState<IPosition | null>(
    null
  );
            ...
            <Group>
              {positions.some((child) => child.parentId === position.id) && (
                <IconChevronDown
                  size={18}
                  style={{
                    transform: expandedNodes.includes(position.id)
                      ? "rotate(180deg)"
                      : "rotate(0deg)",
                    cursor: "pointer",
                  }}
    ...
    <>
      ...
      >
        <Text>
          ...
          {positions.filter(
            (position) => position.parentId === positionToDelete?.id
          ).length > 0 &&
